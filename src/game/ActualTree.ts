...
import {
  AddObjectTagToBlackboard,
  HasSunlight,
  RemoveObjectTagFromBlackboard,
  SetTextureAction,
} from "./TomatoCrop";
...
export class ActualTree extends Phaser.Physics.Arcade.Image {
  ...
  constructor(
    ...
    this.avatar = scene.add
      .image(0, 0, "spritesheet", "Tree" + treeType + "-Shadow")
      .setDepth(2)
      .setOrigin(0.5, 0.5)
    ...
    this.ai = new BehaviorTree(
      new ActiveSelector([
        ...
        new FreshSequence([
          ...
          new RemoveObjectTagFromBlackboard(this, this.blackboard, [
            "tree:stump",
          ]),
          new AddObjectTagToBlackboard(this, this.blackboard, ["tree:grown"]),
          new SetTextureAction(
            this.avatar,
            "spritesheet",
            "Tree" + treeType + "-Shadow"
          ),
        ]),
      ])
    );
